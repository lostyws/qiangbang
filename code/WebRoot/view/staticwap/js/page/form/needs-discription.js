/**
 * Created by yangwenshou on 16/5/31.
 */
seajs.use(['zepto','common','layer.m','loginable','juicer','components/refer-statistics/refer-statistics','user-operation'], function($, Common, Layer, Loginable, Juicer, ReferStatistics ,UserOperation) {
    ...
    var classifyName = decodeURIComponent(Common.getPar('name'));
    var classifyKey = Common.getPar('key');
    var classifyLow = Common.getPar('low');
    console.log(classifyName +'+++++'+classifyKey);
    ...
    $('.select-category').on('click',function(e){
    	e.preventDefault();
    	window.location.href = '/waptask/goChooseCategory';
    })
...
//                    if(dataCategory=="快递"||dataCategory=="跑腿"){
//                    	$(this).attr('onclick',"btnpro(1);");
//                    } else {
//                    	$(this).attr('onclick',"btnpro(0);");
//                    }
//                });
//
...
//        $("#priceNum").attr('placeholder','最低预付'+$(this).attr('minimum')+'元多退少补');
...
//    });
    ...
    if(classifyName&&classifyKey){
    	$("#category").val(classifyName);
    	$('input[name="categoryId"]').val(classifyKey);
    	categoryDom.val(classifyName);




        needArea.val('我需要' + classifyName);
        if(classifyLow){
        	$("#priceNum").attr('placeholder','最低预付'+classifyLow+'元多退少补').attr('minimum',classifyLow);
        }
        //特殊处理显示快递信息
        if(classifyKey == 4 ||classifyKey == 5){
    ...
    function commit_common(){
        ...
        var needInfo = Common.trim($('#needs-title').val());
        if(!needInfo){
        ...
        var priceVal = $('#priceNum').val();
            ...
            var minimum = $('#priceNum').attr('minimum');
