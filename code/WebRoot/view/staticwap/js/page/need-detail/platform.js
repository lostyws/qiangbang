...
seajs.use(['zepto','common','layer.m','user-operation','bridge','bridge-data','task-bottom','calling-download'], function($, Common, Layer, UserOperation, Bridge, BridgeData,taskBottom, callingDownload){
    ...
    $(document).ready(function(){
        $('.provider-msg').on('click', function(){
        	if($(this).attr('data-shopid')!='0'){
        		window.location.href = '/shop/shopDetail/' + $(this).attr('data-shopid');
        	}
        });
    ...
    $('.addagain').on('click', function(e){
        ...
        shadeLayer = Layer.open({
            type: 1,
            content: $('#postageagain').html()
        });
        closeLayer($('.postageagain .cancle, .laymshade'), shadeLayer);
    ...
    //确认收到线下款项（商户）
    $('.confirmoffpay').on('click', function(e){
        e.preventDefault();
        e.stopPropagation();
        ...
        shadeLayer = Layer.open({
            type: 1,
            content: $('#confirm-offpay').html()
        });
        closeLayer($('.confirm-offpay .cancle, .laymshade'), shadeLayer);
  ...
  //确认退款（商户）
    $('.confirmrefund').on('click', function(e){
        ...
        shadeLayer = Layer.open({
            type: 1,
            content: $('#confirm-refund').html()
        });
        closeLayer($('.confirm-refund .cancle, .laymshade'), shadeLayer);
  ...
  //申请完成
    ...
    //确认收到线下支付款项
    $('body').on('click','.confirm-offpay .sure', function(){
        var $this=$(this);
        if($this.hasClass('disabled')){
            return;
        }
        ...
        loadStart($this);
        postJson('/waptask/confirmOffline', data, function(res){
            loadEnd($this);
            if(res.success){
                Common.animaTips(res.data);
                window.location.href = location.href;
    ...
    //同意退款
    $('body').on('click','.confirm-refund .sure', function(){
        var $this=$(this);
        if($this.hasClass('disabled')){
            return;
        }
        ...
        loadStart($this);
        postJson('/waptask/confirmfund', data, function(res){
            loadEnd($this);
            if(res.success){
                Common.animaTips(res.data);
                window.location.href = location.href;
    ...
    $('body').on('click','.select-provider .sure', function(){
        ...
        postJson('/waptask/signing', data, function(res){
            ...
            if(res.success){
//                Common.animaTips(res.data);
                window.location.href = res.href || location.href;
            }else{
            	Common.animaTips(res.data);
            }
            Layer.close(shadeLayer);
    ...
    //中标（选他签单）
    $('body').on('click','.postageagain .sure', function(){
        var $this=$(this);
        if($this.hasClass('disabled')){
            return;
        }
        ...
        loadStart($this);
        postJson('/waptask/againPostPay', data, function(res){
            loadEnd($this);
            if(res.success){
//                Common.animaTips(res.data);
                window.location.href = res.href || location.href;
