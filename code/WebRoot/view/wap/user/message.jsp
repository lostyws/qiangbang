		...
		success: function(d){
			if(d.status == 0){
			    ...
			    for(var i = 0; i < message.length && i < data.pageSize; i++){
			    	...
			    	var isRead = message[i].status == 0 ? '' : ' already';
...
// 			    	var splitxt ="http://";
			    	var splitxt ="#";
			    	var message_url='';
			    	var message_body = message[i].msgcontent.split(splitxt);

			    	if(message_body[1]){
			    		message_url = '<div class="card-footer"><span>'+ message[i].ctime +'</span><a href="javascript:void(0);" data-href="'+(message_body[1])+'"  class="link external" mid="'+message[i].id+'">查看详情<i></i></a></div>'
			    	}
			    	html += 
					    	'<div class="card'+ isRead +'" >'+
								'<div class="card-header">'+ message[i].msgtitle +'</div>'+
								'<div class="card-content">'+
									'<div class="card-content-inner"><a href="javascript:void(0);" data-href="'+(message_body[1])+'" mid="'+message[i].id+'" class="external">'+ message_body[0] +'</a></div>'+
								'</div>'+message_url+
							'</div>';
			    }
